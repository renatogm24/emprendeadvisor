...
class Emprendimiento:
    ...
    def get_emprendimientos(cls, data):
        if "category_id" not in data:
          if "min_promedio" not in data:
            query = "SELECT e.id, e.username, IFNULL(avg(r.rating),0) as promedio, IFNULL(count(r.rating),0) as cuenta, e.full_name, e.biography, e.external_url, e.follower_count, e.is_business, e.public_email, e.contact_phone_number, e.category_name, e.is_private, e.profile_pic_url, e.profile_pic_url_hd, e.category_id, e.created_at, e.updated_at FROM emprendimientos e left join reviews r on e.id = r.emprendimiento_id group by id order by {order_by} DESC limit {offset},{limit};".format(**data)
          else:
          ...
          if "min_promedio" not in data:
            ...
            query = "SELECT e.id, e.username, IFNULL(avg(r.rating),0) as promedio, IFNULL(count(r.rating),0) as cuenta, e.full_name, e.biography, e.external_url, e.follower_count, e.is_business, e.public_email, e.contact_phone_number, e.category_name, e.is_private, e.profile_pic_url, e.profile_pic_url_hd, e.category_id, e.created_at, e.updated_at FROM emprendimientos e left join reviews r on e.id = r.emprendimiento_id left join categories c on e.category_id = c.id group by id having promedio between {min_promedio} and {max_promedio} and cuenta between {min_reviews} and {max_reviews} and e.follower_count between {min_followers} and {max_followers} and e.category_id = {category_id} or c.category_id = {category_id} order by {order_by} DESC limit {offset},{limit};".format(**data)
    ...
    def get_emprendimientos_total_count(cls, data):
        if "category_id" not in data:
          query = "select count(*) as cuenta from emprendimientos;"
        else:
          query = "select count(*) as cuenta from emprendimientos e left join categories c on e.category_id = c.id where e.category_id = %(category_id)s or c.category_id = %(category_id)s;"
        results = connectToMySQL('emprendeadvisor').query_db(query, data)
        if not results:
          return False
        result = results[0]
